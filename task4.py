...
url = "https://" + controller + "/api/v1/host?offset=1" #removed limit variable so that all hosts can be pulled
...
response1 = requests.get(url, headers=header, verify=False)
...
r_resp1=response1.json()
deviceId[]=r_resp1['response'][0]['deviceId']
macAddress[]=r_resp1['response'][0]['macAddress']
portName[]=r_resp1['response'][0]['portName']
result_dict={}
key_tup=()
count=0
for i in range(0,len(deviceId)):
	key_tup=key_tup+deviceId[i]
	key_tup=key_tup+macAddress[i]
	result_dict=result_dict+{key_tup:portName[i]}
	key_tup=()
	count+=1
print(result_dict)
def gethostcount(count):
	print("The total number of hosts are: "+count)
